...
import { GetChildAccount, GetAccountsLevelWise } from "../../../store/apps/ho-coa";
...
import { GetCountries } from '../../../store/apps/country'
import { GetReligions } from "../../../store/apps/religions";
import { GetStates } from '../../../store/apps/state';
...
export const useHoChartOfAccount4thLevel = () => {
  const dispatch = useDispatch();
  const [options, setOptions] = useState([]);
  useEffect(() => {
    const fetchData = async () => {
      const body = { accountLevel: 4 };
      const response = await dispatch(GetAccountsLevelWise(body));
      if (response.payload) {
        const mappedData = response.payload
          .filter((item) => item.nature === "Asset" || item.nature === "Liability")
          .map((item) => ({
            value: item.id,
            label: item.accountCode + " - " + item.accountName,
          }));
        setOptions([{ value: "", label: "-- SELECT ACCOUNT --" }, ...mappedData]);
      }
    };
    fetchData();
  }, [dispatch]);
 ...
 * @property {boolean} [addRight]
 * @property {boolean} [editRight]
 * @property {boolean} [deleteRight]
 * @property {boolean} [viewRight]
 */
...
export const useCountries = () => {
  ...
  return [{ value: "", label: "-- SELECT ACCOUNT --" }, ...filtermap];
}

export const useStates = (countryId) => {
  const dispatch = useDispatch();
  const { data } = useSelector((state) => state.states);
...
export const useReligion = () => {
  const { data, loading } = useSelector((state) => state.religion)
  const dispatch = useDispatch();
  useEffect(() => {
    const fetchData = async () => {
...
export const academicYear = [
  { value: 1, label: "June 2024/25" },
  { value: 2, label: "June 2023/24" },
  { value: 3, label: "June 2022/23" },
];
...
export const recurrenceType = [
  ...
  { value: 2, label: "At Admission Time" }
]
export const months = [
...
export const coaentrylevel = [
  { value: "", label: "Select level" },
  { value: 1, label: "Main Account" },
  { value: 2, label: "Sub Account" },
  { value: 3, label: "Head Account" },
  { value: 4, label: "Detail Account" },
];
