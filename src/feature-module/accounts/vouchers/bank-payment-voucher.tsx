...
import { AddNewVoucher, getSortVouchers, UpdateVoucher, Voucher, AccountOption } from "../../../store/apps/voucher";
...
const BankPaymentVoucher = () => {
    const [voucherTypeId, setVoucherTypeId] = useState(1);
    ...
    const hasPermission = usePermission("Bank Payment Voucher");
    ...
    console.log('voucherHeader:', voucherHeader)
    ...
    const handleEditInitiate = async (id: number) => {
        try {
            setLoadingVoucher(true);
            const res = await axios.get(`${baseURL}/api/HOVoucher/GetById?id=${id}`);
            ...
            setEditVoucherHeader({
                regionId: v.regionId,
                voucherDate: dayjs(v.voucherDate).format('YYYY-MM-DD'),
                isPosted: v.isPosted,
                postedDate: v.postedDate,
                voucherNumber: v.voucherNumber,
                voucherTypeId: v.voucherTypeId,
                voucherYear: v.voucherYear,
            ...
            // Map existing details to the Detail interface (adding arrayid for UI keys)
            const mappedDetails = v.voucherDetailList.map((d: any) => ({
                ...d,
                arrayid: uuidv4(),
            }));
    ...
    const handleUpdateVoucher = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!editVoucherHeader || !editingId) return;

        setUpdateLoading(true);
        try {
            const processedDetails = editDetails.map((d) => ({
                ...d,
                amount: Number(d.amount),
            }));
            const calculatedTotal = processedDetails.reduce((sum, item) => sum + item.amount, 0);

            const payload = {
                ...editVoucherHeader,
                id: editingId, // backend needs the ID for updates
                                ...
                                {hasPermission?.editRight && (
                                    <li>
                                        <Link
                                            ...
                                            onClick={() => handleEditInitiate(record.id)}
                                        >
        ...
        <div>
                ...
                <div className="content">
                    ...
                    <div className="d-md-flex d-block align-items-center justify-content-between mb-3">
                        <div className="my-auto mb-2">
                            <h3 className="page-title mb-1">Bank Payment Voucher</h3>
                            <nav>
                    ...
                    <div className="card">
                        <div className="card-header d-flex align-items-center justify-content-between flex-wrap pb-0">
                            <h4 className="mb-3">Bank Payment Voucher List</h4>
                ...
                <div className="modal-dialog modal-dialog-centered modal-xl">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h4 className="modal-title">Add Bank Payment Voucher</h4>
                            <button
                                type="button"
            ...
            <div className="modal fade" id="edit_expenses">
                <div className="modal-dialog modal-dialog-centered modal-xl">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h4 className="modal-title">Edit Bank Payment Voucher</h4>
                            <button
                                type="button"
                                className="btn-close custom-btn-close"
                                data-bs-dismiss="modal"
                        ...
                        <form onSubmit={handleUpdateVoucher}>
                                                    ...
                                                    <div className="col-md-12">
                                                        <div className="mb-3">
                                                            ...
                                                            <CommonSelect3
                                                                ...
                                                                options={regionsList}
                                                                onChange={(selected) => {
                                                                    // 1. Convert to number (if it exists)
                                                                    const rawValue = selected?.value ? Number(selected.value) : null;

                                                                    // 2. Apply your "if 0 then null" logic
                                                                    const finalValue = rawValue === 0 ? null : rawValue;

                                                                    setEditVoucherHeader(prev => prev ? {
                                                                        ...prev,
                                                                        regionId: finalValue
                                                                    } : null);
                                                                }}
                                                                value={editVoucherHeader?.regionId ? regionsList.find(r => r.value === editVoucherHeader?.regionId) : regionsList[0]}
                                                            />
                                                        </div>
                                                    </div>
                                                )}

                                                {/* Date and Account Cr Section */}
                                                <div className="col-md-6">
                                                    <div className="mb-3">
                                                        <label className="form-label">Voucher Date</label>
                                                        <input
                                                            ...
                                                            onChange={(e) => setEditVoucherHeader(prev => prev ? { ...prev, voucherDate: e.target.value } : null)}
                                                        />
                                                    </div>
                                                </div>
                                                <div className="col-md-6">
                                                    <div className="mb-3">
                                                        ...
                                                        <CommonSelect3
                                                            className="select"
                                                            options={banks}
                                                            value={banks.find((b: any) => b.value === editDetails[0]?.creditAccountId)}
                                                            onChange={(selected: any) => {
                                                                const val = selected?.value || null;
                                                                setEditDetails(prev => prev.map(d => ({ ...d, creditAccountId: val })));
                                                            }}
                                                        />
                                                    </div>
                                                </div>
                                            ...
                                            <div className="add-more-timetable">
                                                <div className="tab-content pt-0 dashboard-tab">
                                                    <div className="tab-pane fade show active">
                                                                ...
                                                                <div className="row timetable-count">
                                                                    <div className="col-lg-3">
                                                                        <div className="mb-3">
                                                                            ...
                                                                            <CommonSelect3
                                                                                options={accounts4thLevel}
                                                                                value={(accounts4thLevel as any[]).find(a => a.value === row.debitAccountId)}
                                                                                onChange={(val: any) => {
                                                                                    setEditDetails(prev => prev.map(d =>
                                                                                        d.arrayid === row.arrayid
                                                                                            ? { ...d, debitAccountId: val?.value ? Number(val.value) : null }
                                                                                            : d
                                                                                    ));
                                                                                }}
                                                                            />
                                                                    ...
                                                                    <div className="col-lg-2">
                                                                        <div className="mb-3">
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                className="form-control"
                                                                                value={row.cheaqueNo || ''}
                                                                                onChange={(e) => setEditDetails(prev => prev.map(d => d.arrayid === row.arrayid ? { ...d, cheaqueNo: e.target.value } : d))}
                                                                            />
                                                                        </div>
                                                                    </div>
                                                                    <div className="col-lg-3">
                                                                        <div className="mb-3">
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                onChange={(e) => setEditDetails(prev => prev.map(d => d.arrayid === row.arrayid ? { ...d, description: e.target.value } : d))}
                                                                            />
                                                                        </div>
                                                                    </div>
                                                                    <div className="col-lg-2">
                                                                        <div className="mb-3">
                                                                            <label className="form-label">Department (opt)</label>
                                                                            {/* Add department dropdown here if needed */}
                                                                        </div>
                                                                    </div>
                                                                    <div className="col-lg-2">
                                                                        <div className="d-flex align-items-end">
                                                                            <div className="mb-3 flex-fill">
                                                                                ...
                                                                                <input
                                                                                    ...
                                                                                    onChange={(e) => {
                                                                                        ...
                                                                                        setEditDetails(prev => prev.map(d => d.arrayid === row.arrayid ? { ...d, amount: numericValue } : d));
                                                                                    }}
                                                                                />
                                                                            </div>
                                                                            <div className="mb-3 ms-2">
                                                                                <Link to="#" className="delete-time-table" onClick={(e) => { e.preventDefault(); setEditDetails(prev => prev.filter(d => d.arrayid !== row.arrayid)); }}>
                                                                                    <i className="ti ti-trash" />
                                                                                </Link>
                                                                            </div>
                                                                        </div>
                                                        ...
                                                        {/* Total and Add Row Section */}
                                                        <div className="add-timetable-row">
                                                            <div className="row timetable-count">
                                                                <div className="col-lg-8"></div>
                                                                <div className="col-lg-2 text-end">
                                                                    <div className="mb-3 mt-2"><strong>Total:</strong></div>
                                                                </div>
                                                                <div className="col-lg-2">
                                                                    <div className="mb-3">
                                                                        <div className="form-control-plaintext">
                                                                            <b>Rs. {editDetails.reduce((sum, item) => sum + Number(item.amount || 0), 0).toLocaleString("en-US")}</b>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <Link
                                                                ...
                                                                onClick={() => setEditDetails([
                                                                    ...editDetails,
                                                                    {
                                                                        arrayid: uuidv4(),
                                                                        debitAccountId: null,
                                                                        creditAccountId: editDetails[0]?.creditAccountId || null,
                                                                        cheaqueNo: '',
                                                                        description: '',
                                                                        amount: 0,
                                                                        // Add these to satisfy the Detail interface:
                                                                        departmentId: 0,
                                                                        referenceId: 0,
                                                                        referenceTypeId: 0,
                                                                        classId: 0,
                                            ...
                                            <div className="row">
                                                <div className="col-lg-6 mt-3 mb-3">
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={handleFileUpload}
                                                    />
                                                    <small className="text-muted">Images will be compressed before upload.</small>

                                                    {editVoucherHeader?.attachment && (
                                                        <div className="mt-3 position-relative" style={{ width: 'fit-content' }}>
                                                            <img
                                                                src={baseURL + '/' + editVoucherHeader?.attachment}
                                                                alt="Attachment"
                                                                className="img-thumbnail"
                                                                style={{ maxHeight: '200px', objectFit: 'cover' }}
                                                            />
                                                            <button
                                                                type="button"
                                                                className="btn btn-danger btn-sm position-absolute top-0 end-0"
                                                                onClick={() => setEditVoucherHeader(prev => prev ? { ...prev, attachment: '' } : null)}
                                                                style={{ borderRadius: '50%', transform: 'translate(50%, -50%)' }}
                                                            >
                                                                &times;
                                                            </button>
                            ...
                            <div className="modal-footer">
                                ...
                                <button type="submit" className="btn btn-success" disabled={updateLoading}>
                                    {updateLoading ? 'Updating...' : 'Update Voucher'}
                                </button>
                            </div>
                        </form>
                    </div>
