...
import { GetSubjects, GetSubject, AddSubject, UpdateSubject, DeleteSubject, Subject } from '../../../store/apps/subjects'
import toast from 'react-hot-toast'

const ClassSubject = () => {
  const routes = all_routes;
  const dispatch = useDispatch<AppDispatch>();
  const hasPermission = usePermission("Subjects");
  const closeBtnRef = useRef<HTMLButtonElement>(null);
  const { data, loading } = useSelector((state: RootState) => state.subjects);
  const [subjectAdd, setSubjectAdd] = useState<Subject>({
    name: '',
    sortOrder: 0
  })
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // ---- VALIDATION ----
    if (!subjectAdd.name.trim()) {
      toast.error("Subject name is required");
      setIsSave(false);
      return;
    }

    if (!subjectAdd.sortOrder || subjectAdd.sortOrder <= 0) {
    ...
    // ---- CALL API ----
    await dispatch(AddSubject(subjectAdd))
      .unwrap()
      .then(() => {
        // Reset input
        setSubjectAdd({
  ...
  const handleEdit = async (id: number) => {
    setEditLoad(true)
    try {
      const res = await dispatch(GetSubject(id)).unwrap();
  ...
  const handleClose = () => {
    setEditSubject({ id: 0, name: "", sortOrder: 0 });
    setSubjectAdd({ name: '', sortOrder: 0 })
  }
  ...
  const handleUpdate = (e: React.FormEvent) => {
    ...
    if (!editSubject.sortOrder || editSubject.sortOrder <= 0) {
      toast.error("Sort order must be greater than 0");
      setIsUpdating(false)
      return;
    }
  ...
  const handleDelete = async () => {
    ...
    await dispatch(DeleteSubject(deleteId))
      ...
      .catch((err) => {
        console.log(err || "Failed to delete subject");
      });
  };
          ...
          <div className="content">
            ...
            <div className="card">
              <div className="card-header d-flex align-items-center justify-content-between flex-wrap pb-0">
                <h4 className="mb-3">Class Subject's</h4>
                  ...
                  <div className="row">
                    <div className="col-md-12">
                      <div className="mb-3">
                        ...
                        <input type="text" value={subjectAdd.name} onChange={handleChange} className="form-control" name="name" placeholder="Enter Subject Name" />
                      </div>
                      <div className="mb-3">
                        <label className="form-label">Order</label>
                        <input type="number" className="form-control" value={subjectAdd.sortOrder} onChange={handleChange} name="sortOrder" placeholder="sort Order" />
                      </div>
                    </div>
                  </div>
                </div>
                      ...
                      <div className="col-md-12">
                        ...
                        <div className="mb-3">
                          ...
                          <input
                            ...
                            value={editSubject.sortOrder}
                            onChange={(e) =>
                              setEditSubject({ ...editSubject, sortOrder: Number(e.target.value) })
                            }
                          />
                        </div>
                      </div>
